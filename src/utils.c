...
/*   Updated: 2025/07/22 22:04:08 by danslav1e        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_all(t_table *tab)
{
	...
	if (tab->mutex_to_stop)
		free(tab->mutex_to_stop);
	if (tab->mutex_min_times_ate)
		free(tab->mutex_min_times_ate);
	if (tab->locks)
		free(tab->locks);
	if (tab->threads)
		free(tab->threads);
	while (i < tab->nb_p)
	{
		free(tab->philos[i].mutex_last_time_ate);
		free(tab->philos[i].mutex_times_ate);
		i++;
	}
	if (tab->philos)
		free(tab->philos);
